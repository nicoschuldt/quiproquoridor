...
function createTestMove(partialMove) {
    return {
        ...partialMove,
...
describe('QuoridorEngine', () => {
     ...
     */
    const createCustomGameState = (players, walls = []) => {
    ...
    beforeEach(() => {
        engine = new QuoridorEngine_1.QuoridorEngine();
    ...
    describe('Création du jeu', () => {
        ...
        it('devrait donner 5 murs à chaque joueur dans une partie à 4', () => {
            const fourPlayerGame = engine.createGame([player1Id, player2Id, player3Id, player4Id], 4);
    ...
    describe('Placement des murs', () => {
        it('devrait placer un mur horizontal de 2 cases', () => {
            ...
            const p1 = newState.players.find((p) => p.id === player1Id);
    ...
    describe('Condition de victoire', () => {
        ...
        it("devrait détecter la victoire dans une partie à 4 joueurs", () => {
            const fourPlayerState = engine.createGame([player1Id, player2Id, player3Id, player4Id], 4);
    ...
    describe('Cas spéciaux', () => {
        it('devrait gérer correctement les bords du plateau', () => {
            ...
            expect(validMoves.some((m) => m.type === 'pawn' &&
                m.toPosition?.x === 0 &&
                m.toPosition?.y === 7)).toBe(true);
            expect(validMoves.some((m) => m.type === 'pawn' &&
                m.toPosition?.x === 1 &&
                m.toPosition?.y === 8)).toBe(true);
            // Pas de déplacement hors plateau (x < 0 ou y > 8)
            expect(validMoves.some((m) => m.toPosition?.x === -1)).toBe(false);
            expect(validMoves.some((m) => m.toPosition?.y === 9)).toBe(false);
        });
        it('devrait gérer correctement les sauts de pions et les déplacements en diagonale', () => {
            ...
            const fourPlayerEng = new QuoridorEngine_1.QuoridorEngine();
            const state4 = fourPlayerEng.createGame([player1Id, player2Id, player3Id, player4Id], 4);
